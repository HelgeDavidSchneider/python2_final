...
def filter_main(img_path, filtertype="square", functiontype="mean", squarelength=None, shape=None, radius=None,
                angles=None):
    '''
    Starting function that takes given path to img and checks parameter types according to chosen filtertype.
    Function will end with error message if provided paramters are not matching expectiations
    '''
...
def img_filter(img_path, filtertype, functiontype="mean", squarelength=None, shape=None, radius=None, angles=None):
    '''
    filter function that calls layerfilter for each color channel according to input paramters set and checked in filter_main
...
def layerfilter(layer, filtertype, functiontype, squarelength, shape, radius, angles):
    ...
    '''
    called by img_filter to return one filtered color channel at a time. Calls filter function accordingly to
    chosen filter type
    '''

    # get padwith (frame around the picture), filtertype as string to be called later and paramters of chosen filtertype
    if filtertype == "square":
